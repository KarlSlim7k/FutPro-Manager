...
import { updateTeamAction, type UpdateTeamActionState } from "@/app/dashboard/leagues/[slug]/teams/[teamSlug]/edit/actions";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { TEAM_STATUS_VALUES, type Team } from "@/types/database";
...
export function EditTeamForm({ leagueSlug, currentTeam }: EditTeamFormProps) {
  const action = updateTeamAction.bind(null, leagueSlug, currentTeam.slug);
  const initialState: UpdateTeamActionState = {
    values: {
      name: currentTeam.name,
      slug: currentTeam.slug,
      logo_url: currentTeam.logo_url ?? "",
      primary_color: currentTeam.primary_color ?? "",
      secondary_color: currentTeam.secondary_color ?? "",
      founded_year: currentTeam.founded_year ? String(currentTeam.founded_year) : "",
  ...
  const [state, formAction, isPending] = useActionState<UpdateTeamActionState, FormData>(
    action,
    initialState
  );
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-gray-500">El slug se usa en la URL del equipo.</p>
      ...
      <div className="space-y-2">
        ...
        />
        {state.fieldErrors.logo_url ? <p className="text-sm text-red-600">{state.fieldErrors.logo_url}</p> : null}
      </div>
