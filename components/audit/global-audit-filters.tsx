"use client";

import { TextLink } from "@/components/ui/text-link";
import { AUDIT_ACTION_OPTIONS, AUDIT_ENTITY_TYPE_OPTIONS } from "@/lib/audit/audit-filters";

interface GlobalAuditFiltersProps {
  currentAction?: string;
  currentEntityType?: string;
  currentActorId?: string;
  currentLeagueId?: string;
  currentFrom?: string;
  currentTo?: string;
  currentQuery?: string;
}

export function GlobalAuditFilters({
  currentAction,
  currentEntityType,
  currentActorId,
  currentLeagueId,
  currentFrom,
  currentTo,
  currentQuery,
}: GlobalAuditFiltersProps) {
  return (
    <form
      method="GET"
      action="/dashboard/audit"
      className="flex flex-wrap gap-3 rounded-xl border border-gray-200 bg-white p-4"
    >
      <div className="flex flex-col gap-1">
        <label htmlFor="global-audit-action" className="text-xs font-medium text-gray-500">
          Acción
        </label>
        <select
          id="global-audit-action"
          name="action"
          defaultValue={currentAction ?? ""}
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-700"
        >
          <option value="">Todas</option>
          {AUDIT_ACTION_OPTIONS.map((action) => (
            <option key={action} value={action}>
              {action}
            </option>
          ))}
        </select>
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="global-audit-entity-type" className="text-xs font-medium text-gray-500">
          Tipo de entidad
        </label>
        <select
          id="global-audit-entity-type"
          name="entityType"
          defaultValue={currentEntityType ?? ""}
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-700"
        >
          <option value="">Todos</option>
          {AUDIT_ENTITY_TYPE_OPTIONS.map((entityType) => (
            <option key={entityType} value={entityType}>
              {entityType}
            </option>
          ))}
        </select>
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="global-audit-actor-id" className="text-xs font-medium text-gray-500">
          ID de actor
        </label>
        <input
          id="global-audit-actor-id"
          name="actorId"
          type="text"
          defaultValue={currentActorId ?? ""}
          placeholder="UUID del actor"
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-700"
        />
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="global-audit-league-id" className="text-xs font-medium text-gray-500">
          ID de liga
        </label>
        <input
          id="global-audit-league-id"
          name="leagueId"
          type="text"
          defaultValue={currentLeagueId ?? ""}
          placeholder="UUID de la liga"
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-700"
        />
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="global-audit-from" className="text-xs font-medium text-gray-500">
          Desde
        </label>
        <input
          id="global-audit-from"
          name="from"
          type="date"
          defaultValue={currentFrom ?? ""}
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-700"
        />
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="global-audit-to" className="text-xs font-medium text-gray-500">
          Hasta
        </label>
        <input
          id="global-audit-to"
          name="to"
          type="date"
          defaultValue={currentTo ?? ""}
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-700"
        />
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="global-audit-query" className="text-xs font-medium text-gray-500">
          Buscar
        </label>
        <input
          id="global-audit-query"
          name="q"
          type="search"
          defaultValue={currentQuery ?? ""}
          placeholder="Acción, entidad o detalle"
          className="rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-700"
        />
      </div>

      <div className="flex items-end gap-2">
        <button
          type="submit"
          className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-700 focus-visible:ring-offset-2"
        >
          Filtrar
        </button>
        <TextLink href="/dashboard/audit" variant="muted">
          Limpiar filtros
        </TextLink>
      </div>
    </form>
  );
}
