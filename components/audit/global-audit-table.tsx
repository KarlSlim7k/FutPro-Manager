...
function getActorLabel(log: GlobalAuditRow): string {
  if (log.actorDisplayName) return log.actorDisplayName;
  if (log.actor_id) return `Usuario ${log.actor_id.slice(0, 8)}`;
  return "Sistema";
}

export function GlobalAuditTable({ logs }: { logs: GlobalAuditRow[] }) {
  return (
    <>
        ...
        {logs.map((log) => (
          <Card key={log.id}>
            <CardContent className="space-y-2 p-4 text-sm text-gray-700">
              ...
              <p className="text-xs text-gray-500">Liga: {log.leagueName ?? "—"}</p>
              <div>
        ...
        <table className="min-w-full divide-y divide-gray-200 bg-white text-sm">
          <thead className="bg-gray-50">
            <tr className="text-left text-gray-500">
              ...
              <th className="px-4 py-3"><Eyebrow as="span">Liga</Eyebrow></th>
              <th className="px-4 py-3"><Eyebrow as="span">Actor</Eyebrow></th>
              <th className="px-4 py-3"><Eyebrow as="span">Acción</Eyebrow></th>
          ...
          <tbody className="divide-y divide-gray-100 text-gray-700">
            {logs.map((log) => (
              <tr key={log.id}>
                <td className="whitespace-nowrap px-4 py-3 text-xs text-gray-500">{formatDateTime(log.created_at)}</td>
                <td className="px-4 py-3">{log.leagueName ?? "—"}</td>
                <td className="px-4 py-3">{getActorLabel(log)}</td>
                <td className="px-4 py-3"><AuditActionBadge action={log.action} /></td>
                <td className="px-4 py-3">{log.entity_type}</td>
                <td className="max-w-xs px-4 py-3"><AuditMetadataPreview metadata={log.metadata} /></td>
              </tr>
            ))}
          </tbody>
        </table>
